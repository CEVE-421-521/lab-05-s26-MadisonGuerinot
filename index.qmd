...
When the dike is 0, there are no construction costs, but the damage is high. As the dike height increases, the expected damages decrease while construction costs increase. Eventually, the total cost of construction outweighs the prevented damage. 
:::
...
1. *How different are the optimal dike heights under RCP 8.5 vs. RCP 2.6?
   ...
   The two heights are not very different. This is surprising because the different scenarios represent very different outcomes.
...
2. *Our planning horizon is 50 years and our discount rate is 3%.
   ...
   (Hint: RCP 8.5 and 2.6 diverge most after 2060-2070, and a 3% discount rate means a dollar of damage 50 years from now is worth only about \$0.22 today.)*
    A shorter planning horizon 50-years means that the full effects of SLR will not be seen yet. This explains why the optimal heights have not diverged yet. The discount rate reduces the costs of future damage, when we see the divergence in scenarios.
...
We could increase the time horizeon, and see the two scenarios diverge more. However, this may not be physically realistic given the lifespan of the dike. Similarly, we could reduce the discount rate if we are concerned we are undervaluing the future avoided damages.
:::
...
This approach gives approximately the same result as running the two scenarios. The ensemble approach is not giving weight to the different SLR trajectories, so it implicitly assumes they are equally likely. This is why the result is about the average of the two SLR scenarios run individually.
...
I would recommend optimizing for an ensemble of scenarios rather than a single scenario. We do not not which scenario is most likely, so it is best to consider a range of options (the ensemble) instead of picking a single one. The limitation of optimizing for a single scenario is that it may not be the most likely or representative of the future, and if we pick the wrong one, we could end up with a suboptimal decision. The limitation of optimizing for an ensemble is that it can be computationally expensive and is less easy to understand and convey to the public. We may care more about other factors, such as the community where the dike will be built and the factors that they care about. 
:::
